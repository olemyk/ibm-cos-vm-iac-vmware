# Packer template for IBM COS Slicestor VM from All-in-One ISO
# Based on successful Manager and Accesser templates
#
# Prerequisites:
...
variable "slicestor_ip" {
  type        = string
  default     = "10.33.3.202"
  description = "Slicestor VM IP address"
}
...
source "vsphere-iso" "cos-slicestor" {
  # vCenter connection
  ...
  # VM configuration
  vm_name              = "cos-slicestor-template"
  notes                = "IBM COS Slicestor template created by Packer from All-in-One ISO"
  guest_os_type        = "other3xLinux64Guest"
  CPUs                 = 2      # 2 vCPUs per IBM docs (unchanged)
  RAM                  = 8192   # 8 GB per IBM docs (unchanged)
  ...
  # Boot configuration
  # Based on successful Manager and Accesser builds: ~17 minutes total
  # Phase 1: Boot menu (0-2 min)
  # Phase 2: Installation (2-5 min)
  # Phase 3: Configuration (5-8 min)
  # Phase 4: Slicestor setup (8-17 min)
  
  boot_wait = "2m"  # Wait 2 minutes for boot menu to appear
  
  boot_command = [
    # ============================================================
    # Phase 1: Boot and Select Slicestor Installation (0-2 minutes)
    # ============================================================
    ...
    "3<enter>",                    # Select option 3: Slicestor component
    ...
    # Configure network
    "channel data port eth0 ip ${var.slicestor_ip} netmask ${var.netmask} gateway ${var.gateway}<enter><wait5>",
    
    # Configure DNS
    "system dns ${var.dns}<enter><wait5>",
    
    # Configure hostname
    "system hostname slicestor<enter><wait5>",
...
# Build
build {
  sources = ["source.vsphere-iso.cos-slicestor"]
  
  # Post-processor to create template
  post-processor "manifest" {
    output     = "manifest-slicestor.json"
    strip_path = true
  }
}