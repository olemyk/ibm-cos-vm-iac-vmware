# Packer template for IBM COS Manager VM from All-in-One ISO
# Based on real installation analysis from doc/ISO-INSTALLATION-ANALYSIS.md
#
# Prerequisites:
# 1. Upload clevos-3.20.1.59-allinone-usbiso.iso to vCenter datastore
# 2. Install Packer: https://www.packer.io/downloads
# 3. Review doc/ISO-INSTALLATION-ANALYSIS.md for installation details
#
# Usage:
...
variable "vcenter_host" {
  type        = string
  default     = ""
  description = "Specific ESXi host to deploy to (optional, e.g., vmware03.oslo.forum.ibm.com)"
}
...
source "vsphere-iso" "cos-manager" {
  ...
  notes                = "IBM COS Manager template created by Packer from All-in-One ISO"
  ...
  storage {
    disk_size             = 262144  # 256 GB - Updated per IBM docs
  ...
  # Boot configuration
  # Based on real installation analysis: ~11 minutes total
  # Phase 1: Boot menu (0-2 min)
  # Phase 2: Installation (2-5 min)
  # Phase 3: Configuration (5-8 min)
  # Phase 4: Manager setup (8-11 min)
  
  boot_wait = "2m"  # Wait 2 minutes for boot menu to appear
  
  boot_command = [
    # ============================================================
    # Phase 1: Boot and Select Manager Installation (0-2 minutes)
    # ============================================================
    ...
    "2<enter>",                    # Select option 2: Manager component
    ...
    # Configure network
    "channel data port eth0 ip ${var.manager_ip} netmask ${var.netmask} gateway ${var.gateway}<enter><wait5>",
    
    # Configure DNS
    "system dns ${var.dns}<enter><wait5>",
    
    # Configure hostname
    "system hostname manager<enter><wait5>",
