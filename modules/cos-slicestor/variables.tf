...
variable "data_disk_count" {
  description = "Number of data disks to attach (minimum 12 for storage pool width 3)"
  type        = number
  default     = 12
}
...
variable "hostname" {
  description = "Hostname for the COS Slicestor"
  type        = string
}

variable "slicestor_number" {
  description = "Slicestor number (1, 2, 3, etc.)"
  type        = number
}
...
variable "deployment_index" {
  description = "Deployment index for sequential deployment (0, 1, 2, etc.)"
  type        = number
}
