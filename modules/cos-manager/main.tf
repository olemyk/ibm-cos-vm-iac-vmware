...
resource "null_resource" "wait_for_ssh" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      echo "Waiting for Manager VM to be accessible via SSH at template IP ${var.template_ip}..."
      ...
      for i in {1..60}; do
        if ssh -i ${var.ssh_private_key} -o StrictHostKeyChecking=no -o ConnectTimeout=5 localadmin@${var.template_ip} "version" 2>/dev/null | grep -q "ClevOS"; then
          echo "Manager VM is accessible at ${var.template_ip}"
          exit 0
        fi
...
# Configure Manager VM via SSH (reconfigure network to new IP)
resource "null_resource" "configure_manager" {
  depends_on = [null_resource.wait_for_ssh]
  
  triggers = {
    vm_id = vsphere_virtual_machine.manager.id
  }
  
  provisioner "local-exec" {
    command = "${path.root}/scripts/configure-manager-ssh.sh"
    environment = {
...
# Verify Manager is accessible at new IP
resource "null_resource" "verify_new_ip" {
  depends_on = [null_resource.configure_manager]
  
  provisioner "local-exec" {
    command = <<-EOT
      echo "Verifying Manager VM is accessible at new IP ${var.ip_address}..."
      sleep 30
      for i in {1..30}; do
        if ssh -i ${var.ssh_private_key} -o StrictHostKeyChecking=no -o ConnectTimeout=5 localadmin@${var.ip_address} "version" 2>/dev/null; then
          echo "Manager VM is accessible at ${var.ip_address}"
          exit 0
        fi
